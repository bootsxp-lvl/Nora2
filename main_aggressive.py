...
# ═══════════════════════════════════════════════════════════════
REQUEST_INTERVAL   = 0.5   # كل نصف ثانية دورة
REQUESTS_PER_WAVE  = 5     # عدد الطلبات المتوازية في كل دورة
SOCKET_TIMEOUT     = 1     # timeout سريع
REQUEST_TIMEOUT    = 3     # timeout للـ API
PREPARE_RETRY_WAIT = 0.5   # الانتظار بين كل محاولة prepare (نصف ثانية)
...
class FF_CLIENT_AGGRESSIVE:
    ...
    def prepare_login_data(self):
        """محاولة واحدة لتحضير البيانات - True/False"""
        ...
        try:
            ...
            RESPONSE = requests.post("https://loginbp.ggpolarbear.com/MajorLogin",
                                     headers=headers, data=Final_Payload,
            ...
            _, _, online_ip, online_port = self.GET_PAYLOAD_BY_DATA(BASE64_TOKEN, self.access_token)
            if not online_ip:
                return False
            ...
            logging.info(f"✅ [{self.account_name}] prepare نجح! OpenID: {verified_open_id}")
            return True
        except:
            return False

    def prepare_until_ready(self):
        """
        يبقى يحاول prepare بدون توقف حتى ينجح.
        كل نصف ثانية محاولة جديدة.
        """
        attempt = 0
        while True:
            attempt += 1
            logging.info(f"⚙️  [{self.account_name}] محاولة prepare #{attempt}...")
            if self.prepare_login_data():
                return  # نجح - اخرج وابدأ الإرسال
            logging.warning(f"⏳ [{self.account_name}] مبند - إعادة المحاولة فوراً (#{attempt})...")
            time.sleep(PREPARE_RETRY_WAIT)

    def send_single_request(self):
        """إرسال طلب واحد"""
        try:
            with self._lock:
                ip    = self.cached_online_ip
                port  = self.cached_online_port
                token = self.cached_final_token
            ...
            if data:
                ...
                logging.info(f"✅ طلب #{count} نجح - [{self.account_name}] Online!")
                return True
            return False
        except:
            return False
    ...
    def aggressive_loop(self):
        """
        كل نصف ثانية → REQUESTS_PER_WAVE طلبات بالتوازي.
        إذا كلها فشلت → prepare من جديد فوراً.
        لا يتوقف أبداً.
        """
        logging.info(f"🔥 [{self.account_name}] بدء الإرسال - {REQUESTS_PER_WAVE} طلب متوازي كل {REQUEST_INTERVAL}ث")
        ...
        while True:
            try:
                ...
                with ThreadPoolExecutor(max_workers=REQUESTS_PER_WAVE) as executor:
                    futures = [executor.submit(self.send_single_request) for _ in range(REQUESTS_PER_WAVE)]
                ...
                # إذا كل الطلبات فشلت → prepare من جديد بلا توقف
                if not any(results):
                    logging.warning(f"⚠️  [{self.account_name}] كل الطلبات فشلت - إعادة prepare...")
                    self.prepare_until_ready()

                # نكمل باقي النصف ثانية
    ...
    def run(self):
        logging.info(f"🚀 [{self.account_name}] بدء التشغيل...")
        logging.info(f"📡 [{self.account_name}] سيبقى يحاول حتى يتفك الباند ثم يبدأ الإرسال فوراً!")
        self.prepare_until_ready()   # مرحلة 1: لا يستسلم أبداً
        self.aggressive_loop()       # مرحلة 2: لا يتوقف أبداً
...
if __name__ == "__main__":
    ...
    print(f"🔥 {REQUESTS_PER_WAVE} طلبات متوازية كل {REQUEST_INTERVAL}ث - بدون توقف أبداً")
    print("=" * 60)
    print()
    ...
    for account in accounts:
        ...
        t = threading.Thread(target=client.run, name=name, daemon=True)
        t.start()
        threads.append(t)
        logging.info(f"🧵 Thread بدأ: {name}")

    try:
        while True:
            time.sleep(1)
    except KeyboardInterrupt:
        logging.info("🛑 إيقاف البرنامج الكامل")
        sys.exit(0)
